...
export function rows(pairs){
  ...
  pairs.forEach(entry => {
    ...
    if (blank && !ed && isSpreadsheet) return;
...
export const linkList = arr => `<ul class="plain">${arr.filter(Boolean)
  .map(x=>`<li><a href="${esc(x)}" target="_blank" rel="noopener">${esc(x)} ↗</a></li>`).join('')}</ul>`;
